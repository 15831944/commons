...
namespace System
{
    ...
    public class HttpHelper
    {
        ...
        private void GetData(HttpItem item, HttpResult result)
        {
            ...
            if (ResponseByte != null && ResponseByte.Length > 0)
            {
                ...
                //得到返回的HTML
                result.Html = this.encoding.GetString(ResponseByte);
            }
            else
            {
                //没有返回任何Html代码
        ...
        private byte[] GetByte()
        {
            ...
            using (MemoryStream _stream = new MemoryStream())
            {
                ...
                {
                    //开始读取流并设置编码方式
                    new GZipStream(this.response.GetResponseStream(), CompressionMode.Decompress).CopyTo(_stream, 10240);
                }
                else
                {
                    //开始读取流并设置编码方式
                    this.response.GetResponseStream().CopyTo(_stream, 10240);
                }
                //获取Byte
        ...
        private void SetPostData(HttpItem item)
        {
            ...
            if (!this.request.Method.Trim().ToLower().Contains("get"))
            {
                ...
                {
                    StreamReader r = new StreamReader(item.Postdata, this.postencoding);
                    buffer = this.postencoding.GetBytes(r.ReadToEnd());
                    r.Close();
                } //写入字符串
    ...
    public class HttpItem
    {
        ...
        {
            get { return this._PostDataType; }
            set { this._PostDataType = value; }
        }
        ...
        {
            get { return this._ResultCookieType; }
            set { this._ResultCookieType = value; }
        }
        ...
        {
            get { return this._ICredentials; }
            set { this._ICredentials = value; }
        }
        ...
        public string RedirectUrl
        {
            get
            {
                try
                {
                    ...
                    {
                        if (this.Header.AllKeys.Any(k => k.ToLower().Contains("location")))
                        {
                            string baseurl = this.Header["location"].ToString().Trim();
                            ...
                            if (!string.IsNullOrWhiteSpace(locationurl))
                            {
                                ...
                                if (!b)
                                {
                                    baseurl = new Uri(new Uri(this.ResponseUri), baseurl).AbsoluteUri;
                                }
                            }
                            return baseurl;
                        }
