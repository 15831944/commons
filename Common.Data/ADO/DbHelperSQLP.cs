...
namespace System.Data.ADO
{
    ...
    public class DbHelperSQLP
    {
        ...
        /// 数据库连接字符串
        /// </summary>
        public string ConnectionString { get; set; } = DBInfo.ConnectionString;

        /// <summary>
        /// Oracle混合事务 Oracle连接字符串
        /// </summary>
        public string OracleConnectionString { get; set; } = DBInfo.OracleConnectionString;
        ...
        public bool ColumnExists(string tableName, string columnName)
        {
            ...
            var res = this.GetSingle(sql);
            return res != null ? Convert.ToInt32(res) > 0 : false;
        }

        public int GetMaxID(string FieldName, string TableName)
        {
            var strsql = "select max(" + FieldName + ")+1 from " + TableName;
            var obj = this.GetSingle(strsql);
        ...
        {
            var obj = this.GetSingle(strSql, cmdParms);
            var cmdresult = Equals(obj, null) || Equals(obj, DBNull.Value) ? 0 : int.Parse(obj.ToString());
            return cmdresult != 0 ? true : false;
        }
        ...
        public int ExecuteSqlTran(List<CommandInfo> list, List<CommandInfo> oracleCmdSqlList)
        {
            using (var conn = new SqlConnection(this.ConnectionString))
            {
                ...
                try
                {
                    ...
                    var res = OracleHelper.ExecuteSqlTran(this.OracleConnectionString, oracleCmdSqlList);
                    if (!res)
                    {
                        tx.Rollback();
